...
The template includes 6 hooks for logging, notifications, and context survival (plus a real git pre-commit hook, `.githooks/pre-commit`, installed via `./scripts/install-hooks.sh`). Git-safety and resource guardrails now live in the machine-wide [`research-guardrails`](../plugins/research-guardrails/) plugin instead, installed once via `./scripts/link-plugin.sh`:
...
| Git guardrails | PreToolUse (command) | `plugins/research-guardrails/hooks/git-guardrails.py` — machine-wide via `./scripts/link-plugin.sh`; blocks destructive git and download-piped-to-shell, prompts on recursive delete |
