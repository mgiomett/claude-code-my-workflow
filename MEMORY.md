...
## Cross-Project Permission Architecture (2026-08-05)

[LEARN:permissions] **Deny and `ask` rules apply in EVERY permission mode, including `bypassPermissions`; allow rules do not.** This is the fact that makes a machine-wide safety floor possible without disturbing a deliberate bypass posture: a floor built from `deny` + `ask` in `~/.claude/settings.json` protects bypass repos and repos with no `.claude/` at all. Corollary: writing `allow` rules at user level is close to pointless — they are inert under bypass and silently widen every future project. Keep allows project-local, floors global.

[LEARN:permissions] **Permissions and hooks need two different distribution mechanisms.** A plugin's `settings.json` accepts only `agent` and `subagentStatusLine`, so permission rules *cannot* ship in a plugin — they must live in `~/.claude/settings.json`. Conversely a hook defined in user settings must not reference `$CLAUDE_PROJECT_DIR` (it resolves to whichever project is open, so every repo without a local copy of the script gets a broken hook); inside a plugin, `${CLAUDE_PLUGIN_ROOT}` resolves correctly everywhere. Hence `plugins/research-guardrails/` + a user-settings floor, not one file.

[LEARN:permissions] **Three permission-rule traps, all confirmed in the docs.** (1) In *user* settings a `/path` rule anchors to `~/.claude/path`, NOT the project — cross-project rules need `//absolute`, `~/home`, or a bare gitignore-style name. (2) Only `Read(path)` and `Edit(path)` are consulted for file access; `Write(...)`/`MultiEdit(...)` path rules are accepted, never used, and warn at startup. (3) Bash rules split compound commands on `|`/`&&`/`;` and match each subcommand independently, so `Bash(curl * | bash)` can never match — pipe-to-shell detection belongs in a hook, which sees the whole command string.

[LEARN:permissions] **Rules resolve deny → ask → allow, and specificity does not reorder them.** A broad deny beats a narrow allow; a matching ask prompts even when a more specific allow also matches. A PreToolUse hook's decision beats an allow rule but **not** a deny or ask rule. Practical consequence, learned the hard way: anything you want to *prompt* on must not also be denied in a hook — the hook deny wins and the ask rule silently never fires. This is why `git merge`/`rebase`/`cherry-pick`/`revert`/`branch -D` were removed from `git-guardrails.py` when they became `ask` rules.

[LEARN:permissions] **A wide allow layer is safe exactly to the degree the layers above it are right.** Because allow is evaluated last, `"Bash"` as a blanket allow does not weaken the boundary — it relocates every real decision into deny/ask/hooks, which are few enough to actually review. The docs endorse this shape directly ("add `Bash` to your allow list and register a PreToolUse hook that rejects those specific commands"). Verified empirically: with `Bash(*)` allowed *and* `bypassPermissions` active, `git-guardrails.py` still blocked a `git reset --hard`.

[LEARN:permissions] **An enumerated allow list causes the per-project accretion it is meant to prevent.** Every command it fails to anticipate becomes a prompt; every prompt becomes a "yes, don't ask again" entry in some project's `settings.local.json`. auroc reached 66 local rules that way — including `Bash(python -c ' *)` (arbitrary Python) and four dead scratchpad paths — and regrew from 5 to 20 in a single session after being pruned. A blanket allow leaves nothing to accrete. Outcome: auroc went from 105 project-specific permission rules to **zero**.

[LEARN:hooks] **A hook whose script is missing fails CLOSED and locks the session — remove the wiring BEFORE deleting the script, never the reverse.** Our hook scripts fail open by design (every internal error exits 0), but that protection only applies once the script *runs*. A `settings.json` entry pointing at a nonexistent path produces `PreToolUse hook error` and blocks the tool call outright. Deleting `.claude/hooks/git-guardrails.py` while `settings.json` still referenced it blocked Bash, Write, Edit, and MultiEdit simultaneously — leaving no in-session way to repair it, because repairing it requires one of those tools. Recovery is `git checkout HEAD -- <path>` from a plain terminal. Correct order when retiring a hook: (1) strip the entry from every `settings.json`, (2) `grep` to confirm no references remain, (3) delete the file. The same trap applies to renaming a hook or moving one into a plugin.

[LEARN:hooks] **A skills-directory plugin can be a symlink.** `~/.claude/skills/<name>/` containing `.claude-plugin/plugin.json` auto-loads as `<name>@skills-dir` in every project with no marketplace and no install step — and it works when that directory is a *symlink* into a git repo (verified 2026-08-05 via `claude plugin list` from an unrelated repo: `Scope: user`, `Status: ✔ loaded`). This is what lets the canonical copy stay version-controlled while loading machine-wide. Undocumented, so re-verify after major Claude Code upgrades.

[LEARN:hooks] **"Any segment is diagnostic" is the wrong predicate for a resource guard.** `resource-guard.py` skipped its check when *any* pipeline segment led with `cat`/`echo`/`head`, so `python3 simulations/big.py | cat` bypassed it entirely. The question is "is this command *purely* diagnostic?" — every segment must qualify. Generalizable: when a guard asks "should I skip this?", `any()` over sub-parts is almost always a bypass; `all()` is almost always what was meant.

[LEARN:workflow] **Guardrail hooks fire on your own test fixtures.** Writing a test for `git-guardrails.py` via a Bash heredoc failed because the heredoc *contained* the denied strings and the live hook matched the command string. Write such fixtures with the Write tool (or assemble the strings at runtime), never inline in a Bash command.

## v1.9.0 Cycle Lessons (2026-05-20, continued)
...
[LEARN:verification] **A self-test you authored is evidence about your imagination, not about your code.** Three independent reviews of one plugin in one session found 8, 6, and 10 defects in work already reported as verified — while its own suite (41 tests, 6 golden fixtures) passed throughout. The reviewer's diagnosis: *every self-test case sits inside the narrow region where the code is correct.* Fixtures encode the inputs you thought of; the bugs live in the ones you did not. Worse, a hand-written cross-check inherits the same blind spot — an independent `grep` audit agreed with the parser at exactly 3494 starred coefficients and **both were wrong by 12**, because the grep reused the parser's own `sym{*` assumption. Two methods agreeing is not evidence when they share an assumption. Practical rules: (1) before claiming something is validated, have an adversary who did not write it try to break it; (2) a cross-check must differ in *assumptions*, not just in code; (3) when reporting a measurement, state which question it answers — in the same session three inflated token-savings ratios all came from quietly substituting a cheaper question than the one asked; (4) treat "the tests pass" as the weakest evidence in the report, not the strongest.
